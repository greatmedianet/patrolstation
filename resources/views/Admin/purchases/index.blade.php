...
<div class="card">
  <div class="card-default">
    <div class="card-header">
      <div class="row">
          <div class="col-sm-3">
              <a href="{{route('purchases.create')}}" class="btn btn-outline btn-primary">Add Purchase</a>
      ...
      <div class="row">
        <div class="col-sm-8">
          <form action="{{route('purchase.reports')}}" method="GET" name="search">
        ...
        @if (Auth::user()->is_super_admin == 1 || Auth::user()->shop_id == 2 )
          <div class="col-sm-4 mt-4">
            <form action="{{ route('szyhpurchaseupload') }}" method="POST" enctype="multipart/form-data" onSubmit="return confirm('Are you sure to send file?')">
              @csrf
              <div class="form-group float-right d-flex">
                  <div class="custom-file text-left mr-2">
        ...
        @if (Auth::user()->is_super_admin == 1 || Auth::user()->shop_id == 2 )
          <div class="col-sm-4 mt-4">
            <form action="{{ route('szyhbpurchaseupload') }}" method="POST" enctype="multipart/form-data" onSubmit="return confirm('Are you sure to send file?')">
              @csrf
              <div class="form-group float-right d-flex">
                  <div class="custom-file text-left mr-2">
                      <input type="file" accept=".csv" name="excel" class="custom-file-input" id="customFile" required/>
      ...
      <div class="card-body">
          ...
          <tbody>
            ...
            <tr>
              ...
              <td>{{$row->Supplier}}</td>
              <td>{{$row->supplierType->name}}</td>
              <td>{{$row->product->name}}</td>
              <td>{{$row->Qty}} Liters</td>
              <td>{{$row->Price}} Kyasts</td>
              <td>{{$row->tank->name}}</td>
              <td>{{$row->Qty * $row->Price}} Kyats</td>
              @if (Auth::user()->is_super_admin == 1)
              <td>{{$row->Shop->name}}</td>
              @endif
              <td>
                <div class="dropdown">
                  <button class="btn btn-info dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    ...
                    <div class="dropdown-menu" aria-labelledby="dropdownMenuButton">
                      <a class="dropdown-item" href="{{route('purchases.edit', $row->id)}}">Edit</a>
                      <form action="{{ route('purchases.destroy', $row->id) }}" method="post" onSubmit="return confirm('Are you sure to delete?')">
                        @csrf
                        @method('DELETE')
